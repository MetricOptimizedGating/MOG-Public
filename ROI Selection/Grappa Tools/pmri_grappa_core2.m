function [R,G]=pmri_grappa_core2(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Edited by CWR on April 14/11 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:length(acc_token)
    ...
    freq_idx(freq_idx==0)=size(obs,2);

    if(~isempty(freq_encode_grappa_block_fraction))
        dd=max(abs(freq_idx-round(size(obs,2)./2)),[],1);
        freq_idx(:,(dd>round(size(obs,2).*freq_encode_grappa_block_fraction/2)))=[];
...
%             acc_block_idx_0=acc_block_idx;
            ...
            acc_block_idx=acc_block_idx(acc_vector==0);
        ...
        T=zeros(length(G{i}.beta{1})/size(freq_idx,1)/size(obs,3),size(freq_idx,1),size(obs,3),'single');

        [dummy,xx]=intersect(G{i}.block_idx,bidx);

        %% Hardcoded for now
        A0=zeros(size(freq_idx,2),size(T,1)*size(T,2)*size(T,3),'single');
